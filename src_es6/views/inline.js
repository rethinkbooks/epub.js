...
class InlineView {
  ...
  lock(what, width, height) {
    const elBorders = core.borders(this.element);
    let iframeBorders;
  ...
  load(contents) {
    ...
    const doc = core.parse(contents, "text/html");
    const body = core.qs(doc, "body");

    const srcs = doc.querySelectorAll("[src]");
    Array.prototype.slice.call(srcs)
      .forEach(item => {
        const src = item.getAttribute('src');
        const assetUri = URI(src);
        const origin = assetUri.origin();
        let absoluteUri;
  ...
  display(request) {
    const displayed = new RSVP.defer();
