...
class EpubCFI {
  ...
  parseStep(stepStr) {
    let type, num, index, has_brackets, id;
  ...
  compare(cfiOne, cfiTwo) {
    ...
    for (let i = 0; i < cfiOne.path.steps.length; i++) {
  ...
  step(node) {
    const nodeType = (node.nodeType === Node.TEXT_NODE) ? 'text' : 'element';
  ...
  filter(anchor, ignoreClass) {
    ...
    let parent, prevSibling, nextSibling;
    let isText = false;
  ...
  */
  ...
  generateChapterComponent(_spineNodeIndex, _pos, id) {
    ...
    const spineNodeIndex = _spineNodeIndex + 1;
