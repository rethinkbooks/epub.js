...
function canonical(doc, section){
  let head;
  let link;
  const url = section.url; // window.location.origin +  window.location.pathname + "?loc=" + encodeURIComponent(section.url);
...
function links(view, renderer) {
  ...
  const replaceLinks = function(link){
    ...
    const linkUri = URI(href);
    const absolute = linkUri.absoluteTo(view.section.url);
    const relative = absolute.relativeTo(this.book.baseUrl).toString();
